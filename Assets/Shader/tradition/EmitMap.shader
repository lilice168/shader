	...
	SubShader {
		Pass{
			...
			float4 frag(vertexOutput i) :COLOR{
					...
					float4 texEmission =tex2D(_EmitMap, i.tex.xy * _EmitMap_ST.xy + _EmitMap_ST.zw);
					...
					float3 lightFinal = UNITY_LIGHTMODEL_AMBIENT.xyz + diffuseReflection + (specularReflection * tex.a) + rimLighting + texEmission.xyz * _EmitStrength;
		...
		Pass{
			...
			float4 frag(vertexOutput i) :COLOR{
					...
					float3 lightFinal = diffuseReflection + (specularReflection * tex.a) + rimLighting + texEmission.xyz * _EmitStrength;
