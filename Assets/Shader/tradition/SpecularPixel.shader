...
Shader "lilice/shader/SpecularPixel"{
	Properties{
	...
	SubShader{
		...
		Pass{
			...
			float4 frag(vertexOutput i):COLOR{
				...
				//vertex function
				float3 normalDirection = mul(float4(i.normalDir, 0.0), unity_WorldToObject).xyz;
				float3 viewDirection = normalize(float3(float4(_WorldSpaceCameraPos.xyz, 1.0) - mul(unity_ObjectToWorld, i.posWorld).xyz));
				...
				return float4(lightFinal * _Color.rgb, 1.0f);
			}
			ENDCG
		}
	}
